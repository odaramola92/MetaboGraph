    ...
    def _run_annotation_process(self):
        ...
        try:
            ...
            if present_annot_cols:
                try:
                    has_any = any(df[c].notna().any() for c in present_annot_cols)
                except Exception:
                    has_any = False
                if has_any:
                    skip_annotation_early = True
                    log_message(f"✅ Detected existing pathway annotation columns: {present_annot_cols}\n")
            ...
            if mode == 'lipid' and not skip_annotation_early:
                ...
                existing_id_cols = [c for c in id_cols_to_check if c in df.columns]
